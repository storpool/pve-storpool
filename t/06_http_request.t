...
mock_lwp_request( 
    ...
    test => sub { # Inject the LWP::UserAgent object
        ...
        my $timeout = $class->timeout > 30 * 60 ? 'timeout' : 'timeout lower than 30 min';
        my $headers = $request->as_string =~ /Authorization: Storpool v1:$token/ ? 'header' : 'header missing';
        my $uri     = $request->uri . "";
        my $content = $request->content;

        return { code => 200, content => encode_json([$timeout,$headers]) }
    }
);
...
is_deeply( $result, ['timeout','header'], 'Timeout and Header set' );
...
is_deeply( $response, {error=>{descr=>'Error code: 404'}}, '404 generic error' );
